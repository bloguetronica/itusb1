...
uint16_t get_raw_current(libusb_device_handle *devhandle)
{
    ...
    int bytes_read;
    if (libusb_bulk_transfer(devhandle, 0x82, read_input_buf, (int)sizeof(read_input_buf), &bytes_read, TR_TIMEOUT) != 0) {  // This bulk transfer should be done regardless of the result of the previous one (version 1.2 fix)
