/* ITUSB1 UDOff Command - Version 2.1 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (err == ITUSB1Device::SUCCESS) {  // Device was successfully opened
        int errcnt = 0;
        std::string errstr;
        device.switchUSBData(false, errcnt, errstr);  // Disconnect the data lines
        if (errcnt > 0) {  // In case of error
            ...
            std::cout << "USB data disabled." << std::endl;
        }
        device.close();
